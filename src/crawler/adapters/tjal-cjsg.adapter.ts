...
// TJAL roda o mesmo e-SAJ do TJSP — mesma estrutura de HTML, mesmo
// parser (verificado ao vivo em 2026-07-19: campos, classes CSS e
// paginação idênticos).
import { parseResultadosCjsg } from '../parsers/tjsp-cjsg.parser';

const FORM_URL = 'https://www2.tjal.jus.br/cjsg/consultaCompleta.do';

export interface TjalCjsgConfig {
...
/**
 * Coleta decisões de 2º grau do TJAL via CJSG (mesmo e-SAJ do TJSP).
 * reCAPTCHA v3, mesma estratégia: browser real via BrowserPoolService.
 */
export class TjalCjsgAdapter implements CrawlerAdapter {
  tribunalSigla = 'TJAL';
  private readonly logger = new Logger(TjalCjsgAdapter.name);

  constructor(
    private readonly browserPool: BrowserPoolService,
    private readonly config: TjalCjsgConfig,
  ) {}
        ...
        for (const item of itens) {
          yield {
            ...
            urlOrigem: item.cdAcordao
              ? `https://www2.tjal.jus.br/cjsg/getArquivo.do?cdAcordao=${item.cdAcordao}&cdForo=0`
              : undefined,
          };
        }
