...
// TJMS roda o mesmo e-SAJ do TJSP — mesma estrutura de HTML, mesmo
// parser (verificado ao vivo em 2026-07-18: campos, classes CSS e
// paginação idênticos).
import { parseResultadosCjsg } from '../parsers/tjsp-cjsg.parser';

const FORM_URL = 'https://esaj.tjms.jus.br/cjsg/consultaCompleta.do';

export interface TjmsCjsgConfig {
...
/**
 * Coleta decisões de 2º grau do TJMS via CJSG (mesmo e-SAJ do TJSP).
 * reCAPTCHA v3, mesma estratégia: browser real via BrowserPoolService.
 */
export class TjmsCjsgAdapter implements CrawlerAdapter {
  tribunalSigla = 'TJMS';
  private readonly logger = new Logger(TjmsCjsgAdapter.name);

  constructor(
    private readonly browserPool: BrowserPoolService,
    private readonly config: TjmsCjsgConfig,
  ) {}
        ...
        for (const item of itens) {
          yield {
            ...
            urlOrigem: item.cdAcordao
              ? `https://esaj.tjms.jus.br/cjsg/getArquivo.do?cdAcordao=${item.cdAcordao}&cdForo=0`
              : undefined,
          };
        }
